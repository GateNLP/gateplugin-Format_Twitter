 ...
 *  
 *  $Id: Population.java 19779 2016-11-24 10:18:32Z markagreenwood $
 */
...
public class Population extends ResourceHelper  {
  ...
  public static void populateCorpus(final Corpus corpus, URL inputUrl, String encoding, List<String> contentKeys,
    ...
    catch (IOException | InvalidOffsetException | PersistenceException | SecurityException e) {
  ...
  private static void closeDocument(Document document, StringBuilder content, Map<PreAnnotation, Integer> annotandaOffsets, Corpus corpus) throws InvalidOffsetException {
    if (content.length() == 0) {
      ...
      for (Map.Entry<PreAnnotation,Integer> entry : annotandaOffsets.entrySet()) {
        try {
          ...
          throw new GateRuntimeException("Attempt to add annotation "+entry.getKey()+" for text="+contentImpl,ex);
        }
      }
      corpus.add(document);
